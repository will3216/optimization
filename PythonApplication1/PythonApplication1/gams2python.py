...
def ConvertedData():
    ...
    for i in range(len(DeclaredIdentifiers)):
        datum = (DeclaredIdentifiers[i], IdentifierTypes[i], IdentifierData[i])
        data.append(datum)
    return data
##############################################################################
...
def input():  
    ...
    # Input file location
    filename = input("Please enter the submission file location: ")
    ...
    # Input the name of the new python script for the model.
    model_file = input("Name the model file to be created \n: ")

    # Passes the model portion of the input to be parsed by the 
    # model_parse function below.
    model_parse(model_solve_split[0], model_file)

    # Input the name of the new python script for the solve portion.
    solve_file = input("Name the solve file to be created \n: ")

    # Passes the solve portion of the input to be parsed by the 
    # solve_parse function below.
    solve_parse(model_solve_split[1], solve_file)
...
#>>>>>>>>>>>>>>>>>>>>>>> SUB-ROUTINES of model_parse() <<<<<<<<<<<<<<<<<<<<<<#
    ...
    # containing the truth values of whether they start with one of the keys
    # in the global variable StatementKeys
    # STATUS: Working
    #=========================================================================
    # EXAMPLE INPUT AND OUTPUT:
    # Given: ['Set woogle', 'Parameter weasle', 'woozle', 'Equation ma',
...
# [X]    Parameter-Scalar
# [X]    Tables
# []     GAMS standard operations conversions
# [X]    Parameter-Direct Assignment
# [X]    Variables
# [X]    Variable Types: Free, Positive, Negative, Binary, Integer
# [X]    Equations
# []     Figure out a way to parse equation data into python code efficiently
...
#       False
# 4. allow for the $ command to input from a .cvs file
# 5. multiple variable and scalar definitions in one line